...
const Transition = forwardRef((props: ModalTransitionProps, ref: Ref<unknown>): JSX.Element => (
...
const ModalDeletarLivro = forwardRef<ModalDeletarLivroRefProps>((_: unknown, ref: Ref<unknown>): JSX.Element => {
    ...
    const deletarLivro = async () => {
        try{
            ...
            adicionarAlerta({
                textoAlerta: `Falha o tentar deletar o livro ${nome}: ${(erro.response.data as ErroApiDTO).mensagem}`,
                tipoAlerta: "error"
            });
        }
    };
    ...
    return (
        ...
        >
            ...
            <DialogContent>
                <DialogContentText>Você deseja deletar o livro <b>{nome}</b> (Código ISBN <b>{adicionarMascaraIsbn(isbn)}</b>)?</DialogContentText>
            </DialogContent>

            <DialogActions>
                <Button onClick={fecharModal} color="warning">Cancelar</Button>
                <Button onClick={deletarLivro} color="success">Deletar</Button>
            </DialogActions>
        </Dialog>
    );
});
