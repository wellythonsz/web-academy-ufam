...
const meuCarrinho = new Carrinho<IProduto>();

// Captura os elementos do DOM
const form = document.getElementById('formProduto') as HTMLFormElement;
const selectTipo = document.getElementById('tipoProduto') as HTMLSelectElement;
const divDinamica = document.getElementById('camposDinamicos') as HTMLDivElement;
...
form.addEventListener('submit', (e) => {
    ...
    let novoProduto: IProduto;

    // Instancia a classe específica baseada na escolha do utilizador
    if (tipo === 'TV') {
        const res = (document.getElementById('resolucao') as HTMLInputElement).value;
        const pol = parseInt((document.getElementById('polegadas') as HTMLInputElement).value);
        novoProduto = new TV(modelo, fabricante, valor, res, pol);
    } else if (tipo === 'Celular') {
        const mem = (document.getElementById('memoria') as HTMLInputElement).value;
...
(window as any).removerDoCarrinho = (index: number) => {
...
function atualizarDisplay() {
    ...
    if (qtdEl) qtdEl.innerText = meuCarrinho.getQuantidade().toString();
    if (valorEl) valorEl.innerText = `R$ ${meuCarrinho.getValorTotal().toFixed(2)}`;
    ...
    meuCarrinho.getItens().forEach((item, index) => {
        ...
        const tipoClasse = (item as any).constructor.name;
