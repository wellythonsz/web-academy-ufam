...
class ToDoListWeb {
    ...
    editar(id: number, novoTitulo: string, novaDataLimite?: Date, novaDescricao?: string): void {
        if (!this.logado) return;
    ...
    getLembrete(id: number): LembreteTupla | undefined {
...
document.getElementById('btnAdicionar')?.addEventListener('click', () => {
    ...
    const dataInput = document.getElementById('dataLimite') as HTMLInputElement;
...
(window as any).iniciarEdicao = (id: number) => {
    const lembrete = app.getLembrete(id);
    if (!lembrete) return;

    // Preenche os campos HTML
    (document.getElementById('titulo') as HTMLInputElement).value = lembrete[1];
    (document.getElementById('descricao') as HTMLInputElement).value = lembrete[4] || '';
    ...
    if (lembrete[3]) {
        ...
        (document.getElementById('dataLimite') as HTMLInputElement).value = `${ano}-${mes}-${dia}`;
    } else {
